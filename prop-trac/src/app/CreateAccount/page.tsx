...
import { ArrowBendUpLeft, Eye, EyeClosed } from "@phosphor-icons/react";
...
const CreateAccount = () => {
  ...
  const [isEight, setIsEight] = useState<boolean>(true);
  const [match, setMatch] = useState<boolean>(true);
  const [disable, setDisable] = useState<boolean>(true);

  const [showPassOne, setShowPassOne] = useState<boolean>(true);
  const [showPassTwo, setShowPassTwo] = useState<boolean>(true);
  ...
  const handleSubmit = async (e: any) => {
    ...
    if (password.length < 8 && testPass.length < 8) {
      return toast({
        variant: "destructive",
        description: "Password must be 8 characters long",
      });
    } else {
      let userInfo = {
        ID: id,
    ...
    <>
      <div className="background h-full md:h-screen w-screen flex">
        <div className="bg-white w-5/6 md:w-2/3 lg:w-2/5 min-h-2/3 m-auto rounded-xl relative max-md:my-6">
          ...
          <p className="text-xl lg:text-2xl text-center pt-10 pb-6">
            Create Account
          </p>
          <div>
            <form className="px-8 md:space-y-2" onSubmit={handleSubmit}>
              <div className="flex max-md:flex-col md:justify-between">
                ...
                <div className="mb-2 block max-md:mx-auto max-md:w-full">
                  ...
                  <TextInput
                    ...
                    required
                  />
                  <p className={`${spec ? "" : "hidden"} text-red-500 text-sm`}>
              ...
              <div className="flex max-md:flex-col md:justify-between">
                <div className="mb-2 block max-md:mx-auto max-md:w-full">
                  ...
                  />
                  <p
                    className={`text-red-600 text-[10px] pl-1 pt-1 ${
                      isEight ? "hidden" : ""
                    }`}
                  >
                ...
                <div className="mr-3 flex justify-between">
                  ...
                  <TextInput
                    ...
                    onChange={(e) => {
                      e.target.value === testPass
                        ? (setPassword(e.target.value), setMatch(true))
                        : setMatch(false);
                    }}
                    required
                  />
              ...
              <div className="flex max-md:flex-col md:justify-between">
                <div className="mb-2 block max-md:mx-auto max-md:w-full">
                  ...
                  <Select
                    ...
                    id="securityQ"
                    onChange={(e) => {
                      setSecurityId(parseInt(e.target.value));
                    }}
                    required
                  >
                    {securityArray &&
                      securityArray.map((question, idx) => (
              ...
              <div className="flex justify-center md:justify-end pt-2 pb-7">
                <Button
                  ...
                  color="light"
                  type="submit"
                  disabled={isEight && spec ? false : true}
                >
                  Create Account
                </Button>
              </div>
