...
import { Bathtub, Bed, CaretCircleLeft, CaretRight, NotePencil, Trash } from '@phosphor-icons/react'
  ...
  const handleDelete = async() => {
    ...
    let propToDel = {
      ...
      if(!success){
        ...
        toast({description: 'Property Successfully Deleted', variant:'default'})
        router.push('/Properties')
      }
  }
  ...
  return (
    ...
    <div className={`${isOpen ? 'block' : 'hidden'}`}>
            ...
            <div className=' bg-white rounded-xl w-11/12 md:w-3/5 lg:w-1/2 xl:w-1/3 min-h-[400px] mx-auto left-5 md:left-[20%] lg:left-[25%] xl:left-[35%] top-[10%] md:top-[20%] fixed'>
                <p className={`${first || second ? 'hidden' : 'block'} text-center text-xl py-4`}>Edit Property</p>
        ...
        <div className='lg:ml-52 pt-6'>
          ...
          <div className='flex max-md:flex-col max-lg:justify-center md:space-x-6 max-lg:mx-auto xl:space-x-12'>
            <div className='w-11/12 max-md:mx-4 md:w-2/5'>
              ...
              <button className='flex bg-[#E5E5E5] px-3 py-2 rounded-xl' onClick={() => setIsOpen(true)}>
                <NotePencil size={24} className='my-auto mr-1'/>
                <p className='my-auto text-xs lg:text-sm xl:text-base'>Edit Property</p>
              </button>
              <button className='flex bg-[#DD7575] px-3 py-2 rounded-xl' onClick={handleDelete}>
